...
def login():
   ...
   # Comprobar si existen solicitudes POST de "nombre de usuario" y "contraseña" (formulario enviado por el usuario)
    if request.method == 'POST' and 'primer_nombre' in request.form and 'contrasena' in request.form:
        primer_nombre = request.form['primer_nombre']
        contrasena    = request.form['contrasena']
        print(contrasena)
 
        # Verificar si la cuenta existe usando SQL
        cursor.execute('SELECT * FROM empleado WHERE primer_nombre = %s', (primer_nombre,))
...
def registro_empleados():
    ...
    return render_template('registro_empleado.html', list_users = list_users) 
...
def productos():
    ...
    s    = "SELECT producto.id, producto.nombre, producto.precio_venta, producto.cantidad, categoriat.nombre_categoria from producto, categoriat WHERE categoriat.id_categoria = producto.id_categoria"
...
def salida_productos():
    cur  = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    curc = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    s    ="SELECT venta.id_venta, venta.fecha_venta, venta.precio, venta.cantidad, producto.nombre from producto, venta where venta.id_producto = producto.id"
...
def entrada_productos():
    ...
    s    = "SELECT compra.id_compra, compra.fecha_compra, compra.precio_compra, compra.cantidad, producto.nombre, proveedor.nombre, empleado.primer_nombre from producto, compra, proveedor, empleado where compra.id_producto = producto.id AND compra.id_proveedor = proveedor.id AND compra.id_usuario = empleado.id"
    sc   = "SELECT * FROM public.producto"
